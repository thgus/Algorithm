...
{
	auto dist = m.find(dist_str);
	long long len = abs(taxi.y - dist->second.y) + abs(taxi.x - dist->second.x);
	...
	while (!taxi.passenger.empty())
	{
		...
		p.len += abs(dist->second.y - taxi.y) + abs(dist->second.x - taxi.x);
		if (p.dist == dist_str)
		{
			taxi.money += p.len * b;
		}
	...
	if (dist->second.isGasSatation)
	{
		long double left_len = (a * c) - taxi.can_go;
		left_len += 1e-12;

		long double cost = ((long double)left_len / (long double)c);
		cost = cost + 1e-12;

		long double dist_cost = dist->second.cost;
		long long need_cost = dist_cost * cost;
		...
		{
			taxi.can_go += (long double)(taxi.money) / (dist_cost);
			taxi.can_go += 1e-12;
			taxi.money = 0;
		}
	}
	...
	taxi.cur = dist_str;
	taxi.y = dist->second.y;
	taxi.x = dist->second.x;
